...
import { cartBtn, cartBtnCount, cartCount, cartItems, cartTotalAmount } from "./selector";
...
export const totalCartAmount = () => {
  const total = [...cartItems.querySelectorAll(".cart-cost")].reduce((pv,cv) => pv + parseFloat(cv.innerText),0);
...
export const addToCart = (e) => {
  ...
  const currentProduct = products.find((el) => el.id === parseInt(currentProductId));
  ...
  document.body.append(newImg);
    ...
    const keyFrame = [
      ...
      { top:icon.top + "px" , left:icon.left + "px" , width:0 , rotate:360 + "deg"},
    ]
...
export const updateQuantity = (e,q) => {
  ...
  if(q > 0 || currentQuantity.innerText > 1){
    ...
    cost.innerText = (parseInt(currentQuantity.innerText) * price.innerText).toFixed(2);
  }
  // totalCartAmount();
}
...
export const orderFunction = () => {
  ...
  const productId = id;
  const productQuantity = parseInt(el.querySelector(".cart-quantity").innerText);
